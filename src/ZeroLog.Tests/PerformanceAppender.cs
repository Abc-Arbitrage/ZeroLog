...
internal class PerformanceAppender : Appender
{
    ...
    private struct MessageReceived(char[] startTimestampInChars)
    {
        public readonly char[] StartTimestampInChars = startTimestampInChars;
    ...
    private static double ToMicroseconds(long ticks)
        => unchecked(ticks * 1000000 / (double)Stopwatch.Frequency);
}
